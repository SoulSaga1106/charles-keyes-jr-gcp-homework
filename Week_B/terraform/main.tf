#wutang: A firewall rule so port 80 can sing to the world.
resource "google_compute_firewall" "wutang_allow_http" {
  name    = "wutang-allow-http"
  network = "default"
...
#wutang: The startup script is your first automation spell.
locals {
  startup_script = <<-EOT
    #!/bin/bash
    set -euo pipefail

    #wutang: This node serves proof-of-life.
    apt-get update -y
    ...
    <body>
      <div class="wrap">
        ...
        <p>#wutang: You didn’t click your way here. You automated.</p>
      </div>
    </body>
    </html>
    EOF
...
#wutang: The compute instance—your first reproducible node.
resource "google_compute_instance" "wutang_vm" {
  name         = var.vm_name
  ...
  metadata = {
    #wutang: The banner is identity. Make it yours.
...
#wutang: Outputs are how automation speaks to other automation.
output "vm_external_ip" {
  value = google_compute_instance.wutang_vm.network_interface[0].access_config[0].nat_ip
}

output "vm_url" {
  value = "http://${google_compute_instance.wutang_vm.network_interface[0].access_config[0].nat_ip}"
}
